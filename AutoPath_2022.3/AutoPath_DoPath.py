...
class DOPATH:
    ...
    def do_pendulum_return(self, select, dirvalue, towervalue, chainpath, rollerpath, step, chainbracing, carnum, pitch,
                           swingleng):
        """摆杆 - 轨迹/动画"""
        self.dirvalue = dirvalue
        steppnt = self._pathpnt(chainpath, step)
        ...
        for i in steppnt:
            ...
            nowdist = (self.j - 1) * float(step) + self.frtstep
            if towervalue == 1:
                distcompare = chainbracing + swingleng + (carnum - 1) * pitch
            else:
                distcompare = chainbracing * 2 + (carnum - 1) * pitch
            if nowdist > distcompare:
                ...
                for num in range(carnum):
                    ...
                    if select == 3:  # 返回段轨迹
                        swingnum = 0
                        inspnt1 = inspnt0
                        self._find_insertpnt(chainpath, chainbracing, nowdist - num * pitch, inspnt1, i)
                        self._insert_block('ChainPlate', chainplate, select)
                        self.circlebr.Delete()
                        self.swingpnt.append(self._find_isotrianglepnt(inspnt1, select))
                        if swingnum == 0:  # 前摆杆
                            self._insert_swing(fswing, self.swingpnt[0], swingleng, -1, 'FSwing')
                        elif swingnum == 1:  # 后摆杆
                            self._insert_swing(bswing, self.swingpnt[1], swingleng, bracing, 'BSwing')
                        else:
                            swingpnt0 = Tc.vtpnt(self.swingpnt[-1][0], self.swingpnt[-1][1])
    ...
    def _insert_block(self, block_name, blocklist, select=1):
        """插入块并调整角度，默认为工艺段，返回段需传入select"""
        if self.dirvalue == 1:  # 工件向右
            a = self.angbr[self.index]
            blocklist.append(self.msp.InsertBlock(self.inspnt, block_name, 1, 1, 1, a))
            if self.mirmark == 1 and select != 1:  # 起始向左
            ...
            if self.angbr[self.index] < math.pi:
                ...
                a = self.angbr[self.index] - math.pi
            blocklist.append(self.msp.InsertBlock(self.inspnt, block_name, 1, 1, 1, a))
            if self.mirmark == 0 and select != 1:  # 起始向右
                fstpnt = Tc.vtpnt(self.intpnts[self.index * 3], self.intpnts[self.index * 3 + 1])
                blockmirror = blocklist[-1].Mirror(fstpnt, self.inspnt)
                blocklist[-1].Delete()
                blocklist.append(blockmirror)
    ...
    def _find_isotrianglepnt(self, inspnt0, height=140, direction=0):
        """已知等腰三角形两底角坐标和高，求顶角坐标。默认为工艺段，返回段需传入select"""
